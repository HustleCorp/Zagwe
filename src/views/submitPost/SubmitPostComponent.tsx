...
export class SubmitPost extends Component<
  ISubmitPostComponentProps,
  ISubmitPostComponentState
> {
  quillRef: any;
  reactQuillRef: any;
  formats: string[];
  ...
  constructor(props: ISubmitPostComponentProps) {
    ...
    this.state = {
       ...
       */
      postBodyText:
        this.props.edit && EditPost ? EditPost.get('bodyText', '') : '',
      ...
      postTopic:
        this.props.edit && EditPost
          ? TopicsFUll[TagsMap[EditPost.get('postTopic', '')]]
          : '',
      ...
      imagefullPath:
        this.props.edit && EditPost ? EditPost.get('imageFullPath', '') : '',
  ...
  submitPost = () => {
    ...
    const {uid, ownerAvatar, ownerDisplayName, post, goTo} = this.props;

    if (postTitle.length < 5 || postTitle.length > 80) {
      this.setState({
        postTitleError: 'Title must be more than 5 characters long',
      });
      error = true;
      ...
      this.setState({
        postBodyError: 'Post is too short, please add more content',
      });
      error = true;
    }
    if (!error) {
      if (!edit) {
        ...
        FileAPI.getThumbUrl(imagefullPath!).then(url => {
          if (url) {
            ...
            update!(updatedPost, (postId: string) => {
              goTo!(`/posts/${uid}/${postId}`);
            });
          } else {
            const updatedPost = EditPost!
  ...
  renderTag = (props: any) => {
    ...
    return (
      <span
        ...
        {...other}
        onBlur={() => {
          this.setState({isTagFocus: false});
        }}
      >
        {
          <span
            onClick={() => {
              this.setState({isTagFocus: true});
            }}
          >
  ...
  render() {
    ...
    const error =
      this.state.postTitleError.trim() !== '' ||
      this.state.postCatagoryError.trim() !== '';
    if (error) {
      this._input.focus();
    }
    return (
      <div className={classes.submitPostContent}>
        <Prompt
          when={
            (this.state.postBodyHTML !== '' || this.state.postTitle !== '') &&
            this.state.willSubmit === false
          }
          message={
            'Are you sure you want to leave the page? any unsaved items will be lost'
          }
        />
        ...
        <div style={{color: 'red'}}>
          {this.state.postBodyError.trim() !== ''
            ? 'Post is too short, please add more content'
            : ''}
        </div>
        <ReactQuill
          ref={el => {
          ...
          <a>
            <div style={{paddingTop: '10px', display: 'flex'}}>
              <TagsInput
                ...
                initval={
                  this.props.edit && this.props.EditPost
                    ? this.props.EditPost.get('tags', '')
                    : List([])
                }
              />
              <div>
                <Tooltip
                  title={
                    'Tags help make your posts more discoverable and show up in searchs more frequently'
                  }
                >
                  <IconButton>
                    <Question />
        ...
        >
          <DialogContent>
            <DialogContentText>
              <h3>
                {' '}
                Can I post on Zagwe even if I am not a professional writer?{' '}
              </h3>

              <p>
                {' '}
                YES. Zagwe is a platform for anyone who wants to share their
                written work, whether you are a professional writer or took up
                writing as a hobby, you are welcome to contribute.{' '}
              </p>
              ...
              <p>
                {' '}
                Trending posts are the posts that have the most views and
                engagement. Check for any grammar and spelling mistakes before
                you submit your post. Include a featured image with your posts
                for better appeal and engage with your audience{' '}
              </p>

              <h3>
                {' '}
                How can I increase my followers and expand my reach on Zagwe?{' '}
              </h3>

              <p>
                Find more Zagwe followers under the 'Discover' page. Share the
                link to your Zagwe posts on your other social media platforms
                (facebook, twitter…) to encourage your friends and family to
                join and follow you! You can invite your contacts to join Zagwe
                from your profile.
              </p>

              <h3>
                {' '}
                I can’t find a category that fits my work. What should I do?
              </h3>

              <p>
                For any posts that don't fit the categories listed, you can
                submit the post under the 'Other' category. The Zagwe team will
                add new categories if enough posts are made.
              </p>

              <h3>How can I reach a member of the Zagwe team? </h3>

              <p>
                If you have any feedback, we would love to hear from you via our
                feedback form. For any other questions or concerns, contact us
                at info@Zagwe.io.
              </p>
            </DialogContentText>
          </DialogContent>
        ...
        >
          <DialogContent>
            <DialogContentText>
              <p>
                Zagwe’s main purpose is to promote both informative and creative
                Ethiopian writing. Violence, harassment, online trolling, and
                other similar types of behavior ultimately diminish the value
                that the platform brings, and as such, all users are obliged to
                follow the submission guidelines outlined below. Accounts or
                posts who fail to do so will be suspended and their posts taken
                down.
              </p>
              <h3> Hate speech: </h3>
              <p>
                You may not use Zagwe’s services to express prejudice against a
                particular group on the basis of ethnicity, religion, or sexual
                orientation.
              </p>

              <h3> Misinformation/ disinformation: </h3>
              <p>
                You may not spread false or biased information - with or without
              ...
              <p>
                ...
                Impersonation and 'fan pages' who post the work of a contributor
                without their knowledge and consent are also prohibited.
              </p>
              <h3>Ethnic violence and extremism:</h3>
              <p>
                You may not use Zagwe’s services for the purpose of spreading
                ethnic violence or ideologies of ethnic extremism. Zagwe is
                committed to maintaining a safe space and takes this matter
                seriously.
              </p>
              <h3>Sensitive media:</h3>
              <p>
                You may not submit posts that include graphic or adult content
                excessively. Posts depicting sexual violence and/or assault are
                also not permitted.
              </p>
              <h3>Abuse/ Harassment:</h3>
              <p>
                You may not engage in the targeted harassment of someone due to
                their gender, profession, or their posts on Zagwe including any
                prior written work.
              </p>
            </DialogContentText>
          </DialogContent>
...
const mapDispatchToprops = (
  dispatch: any,
  ownProps: ISubmitPostComponentProps,
) => {
  return {
    ...
    uploadImage: (image: any, imageName: string) =>
      dispatch(imageGalleryActions.dbUploadImagePost(image, imageName)),
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToprops,
  )(withStyles(styles as any)(SubmitPost as any) as any) as any,
) as any;
