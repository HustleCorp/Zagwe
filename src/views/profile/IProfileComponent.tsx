...
> {
  ...
  loadPost = () => {
    ...
    return (
      <St posts={posts} loadStream={loadPosts} hasMorePosts={hasMorePosts} />
    );
  };
      ...
      <div style={styles.profile}>
        <div style={styles.header}>
          <AsyncHeader
            ...
            isAuthedUser={this.props.isAuthedEdit!}
              ...
              <span>
                <div>
                  <NavLink
                    ...
                    className={classes.indiv}
                    style={{color: tab === 'posts' ? '#111' : '#999'}}
                  >
                    <span className={classes.spanNum}>
                      {posts ? posts.size : 0}
                    </span>
                    <span className={classes.spanValue}>posts</span>
                  </NavLink>
                  <NavLink
                    to={`/users/${userId}/likes`}
                    className={classes.indiv}
                    style={{color: tab === 'likes' ? '#111' : '#999'}}
                  >
                    <span className={classes.spanNum}>
                      {likes ? likeCount : 0}
                    </span>
                    <span className={classes.spanValue}>likes</span>
                  </NavLink>
                </div>
              </span>
            </div>
...
const mapDispatchToProps = (
  ...
  return {
    ...
    loadUserOther: () =>
      dispatch(userActions.dbGetUserOtherInfoByUserId(userId, 'header')),
...
const mapStateToProps = (
  ...
  const otherProfile = state.getIn(
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles as any)(ProfileComponent as any));
