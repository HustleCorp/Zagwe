...
export const detectTags: (content: string, character: string) => string[] = (
  content: string,
  character: string,
) => {
...
export const sortByPopularity = (objects: any) => {
  let sortedObjects = objects;
...
export const sortImuObjectsDate = (objects: List<Map<string, any>>) => {
  let sortedObjects = objects;
  // Sort posts with creation date
  return sortedObjects.sort((a: Map<string, any>, b: Map<string, any>) => {
    return (
      parseInt(b.get('creationDate'), 10) - parseInt(a.get('creationDate'), 10)
    );
  });
};
