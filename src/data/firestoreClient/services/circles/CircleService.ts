...
export class CircleService implements ICircleService {
  public addCircle: (userId: string, circle: Circle) => Promise<string> = (
    userId,
    circle,
  ) => {
  ...
  public deleteCircle: (userId: string, circleId: string) => Promise<void> = (
    userId,
    circleId,
  ) => {
    return new Promise<void>((resolve, reject) => {
  ...
  public getCircles: (
    userId: string,
  ) => Promise<{[circleId: string]: Circle}> = userId => {
