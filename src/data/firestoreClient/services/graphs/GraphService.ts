...
export class GraphService implements IGraphService {
   ...
   * Add graph
   */
  public addGraph: (graph: Graph, collection: string) => Promise<string> = (
    graph,
    collection,
  ) => {
   ...
   * Update graph
   */
  public updateGraph: (graph: Graph, collection: string) => Promise<string> = (
    graph,
    collection,
  ) => {
    return new Promise<string>((resolve, reject) => {
      const graphData = this.getGraphs(
        collection,
        graph.leftNode,
        graph.edgeType,
        graph.rightNode,
  ...
  public getGraphs: (
    ...
    return new Promise<Graph[]>((resolve, reject) => {
      this.getGraphsQuery(collection, leftNode, edgeType, rightNode).then(
        result => {
   ...
   */
  public deleteGraphByNodeId: (nodeId: string) => Promise<void> = nodeId => {
