...
export class VoteService implements IVoteService {
  public addVote: (vote: Vote) => Promise<string> = vote => {
  ...
  public getVotes: (
    ...
    return new Promise<{[postId: string]: {[voteId: string]: Vote}}>(
      (resolve, reject) => {
        let votesRef = db.doc(`posts/${postId}`).collection(`votes`);

        votesRef.onSnapshot(snapshot => {
          let parsedData: {[postId: string]: {[voteId: string]: Vote}} = {
            [postId]: {},
          };
  ...
  public deleteVote: (userId: string, postId: string) => Promise<void> = (
    userId,
    postId,
  ) => {
