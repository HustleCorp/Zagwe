...
export class NotificationService implements INotificationService {
  ...
  public sendTokenTodb: (userId: string, token: string) =>
       Promise<void> = (userId, token) => {
          return new Promise<void>((resolve, reject) => {
              const dataref = db.collection('tokens').doc(`${userId}`)

              dataref.get().then((data: any) => {
                    if (data.exists) {
                        const tokens = data.data().token as string []
                        return [...tokens, token]
                    } else {
                       return [token]
                    }
                     }).then((tokens: string []) => {
                        db.collection('tokens').doc(`${userId}`).set({token: tokens}).then(() => { resolve()}).catch((error: any) => {
                          reject(new SocialError(error.code, error.message))
                              })
              })
          })
