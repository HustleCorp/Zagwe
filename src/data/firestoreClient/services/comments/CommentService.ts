...
export class CommentService implements ICommentService {
   ...
   */
  public addComment: (comment: Comment) => Promise<string> = comment => {
  ...
  public getComments = (
    postId: string,
    next: (resultComments: Map<string, Map<string, any>>) => void,
  ) => {
    ...
    const unsubscribe = commentsRef.onSnapshot(snapshot => {
      ...
      snapshot.forEach(result => {
        parsedData = parsedData.setIn(
          [postId, result.id],
          fromJS({id: result.id, ...result.data()}),
        );
      });
      if (next) {
        next(parsedData);
   ...
   * @memberof CommentService
   */
  public updateComment: (comment: Comment) => Promise<void> = comment => {
   ...
   * @memberof CommentService
   */
  public deleteComment: (commentId: string) => Promise<void> = commentId => {
    return new Promise<void>((resolve, reject) => {
