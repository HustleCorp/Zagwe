...
export class PostService implements IPostService {
  public addPost: (post: Post, postBody: Post) => Promise<string> = (
    post,
    postBody,
  ) => {
   ...
   * Updare post
   */
  public updatePost: (post: Post, postBody: Post) => Promise<void> = (
    post,
    postBody,
  ) => {
   ...
   * Delete post
   */
  public deletePost: (postId: string) => Promise<void> = postId => {
    return new Promise<void>((resolve, reject) => {
      const batch = db.batch();
  ...
  public getFeaturedPosts: () => Promise<{
    posts: {[postId: string]: Post}[];
  }> = () => {
    return new Promise<{posts: {[postId: string]: Post}[]}>(
      (resolve, reject) => {
  ...
  public getAllPostsByTopic: (
    header: string,
    lastPostId: string,
    page: number,
    limit: number,
  ) => Promise<{posts: {[postId: string]: Post}[]; newLastPostId: any}> = (
    header,
    lastPostId,
    page = 0,
    limit = 5,
  ) => {
    return new Promise<{posts: {[postId: string]: Post}[]; newLastPostId: any}>(
      (resolve, reject) => {
        let parsedData: {[postId: string]: Post}[] = [];
        ...
        query.get().then((posts: any) => {
          let newLastPostId =
            posts.size > 0 ? posts.docs[posts.docs.length - 1] : '';
  ...
  public getPosts: (
    ...
    return new Promise<{
      ...
      // Get user ties
      db.collection('graphs:users')
        .where('leftNode', '==', currentUserId)
        .get()
        .then(tieUsers => {
          if (!(tieUsers.size > 0)) {
            // Get current user posts
            this.getPostsByUserId(currentUserId, lastPostId, page, limit).then(
              result => {
                resolve(result);
              },
            );
              ...
              this.getPostsByUserId(userId).then(posts => {
                ...
                if (userCounter === tieUsers.size) {
                  ...
                  this.getPostsByUserId(currentUserId).then(result => {
                    postList = [...postList, ...result.posts];
  ...
  public getPostsByUserId: (
    ...
    return new Promise<{
      ...
      query.get().then(posts => {
        let newLastPostId =
          posts.size > 0 ? posts.docs[posts.docs.length - 1].id : '';
        posts.forEach(postResult => {
          const post = postResult.data() as Post;
          parsedData = [
            ...parsedData,
   ...
   */
  public getPostById: (postId: string) => Promise<Post> = postId => {
  ...
  public getPostBodyById: (postId: string) => Promise<Post> = postId => {
    return new Promise<Post>((resolve, reject) => {
  ...
  private pagingPosts = (
    ...
    if (lastPostId && lastPostId !== '') {
      ...
      sortedObjects = sortedObjects.slice(
        lastPostIndex + 1,
        lastPostIndex + limit + 1,
      );
    ...
    const newLastPostId =
      sortedObjects && sortedObjects.length > 0
        ? Object.keys(sortedObjects[sortedObjects.length - 1])[0]
        : '';
