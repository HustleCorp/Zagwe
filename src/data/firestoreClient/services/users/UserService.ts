...
export class UserService implements IUserService {
      ...
      userOtherProfileref
        ...
        .catch((error: any) =>
          reject(
            new SocialError(
              error.code,
              'firestore/getUserOther :' + error.message,
            ),
          ),
        );
    });
   ...
   * Get user profile
   */
  public getUserProfile: (userId: string) => Promise<Profile> = userId => {
    return new Promise<Profile>((resolve, reject) => {
      ...
      userProfileRef
        ...
        .then(result => {
          if (!result.exists) {
            this.getUserProviderData(userId).then(
              (providerData: UserProvider) => {
                if (!UserProvider || !providerData.email) {
                  reject(
                    reject(
                      new SocialError(
                        `firestore/providerdata`,
                        'firestore/getUserProfile : Provider data or email of provider data is empty!',
                      ),
                    ),
                  );
                }
        ...
        .catch((error: any) =>
          reject(
            new SocialError(
              error.code,
              'firestore/getUserProfile :' + error.message,
            ),
          ),
        );
    });
      ...
      batch
        ...
        .catch((error: any) =>
          reject(
            new SocialError(
              error.code,
              'firestore/updateUserProfile' + error.message,
            ),
          ),
        );
    });
  ...
  public getUsersProfile: (
    ...
    newLastUserId: string;
  }> = (userId, lastUserId, page, limit = 10) => {
    return new Promise<{
      users: {[userId: string]: Profile}[];
      newLastUserId: string;
    }>((resolve, reject) => {
      ...
      query
        ...
        .then(users => {
          let newLastUserId =
            users.size > 0 ? users.docs[users.docs.length - 1].id : '';
        ...
        .then(snapshot => {
          if (snapshot.exists) {
            ...
            throw new SocialError(
              `firestore/getUserProviderData/notExist `,
              `document of userProviderRef is not exist `,
            );
          }
        })
        .catch((error: any) => {
          reject(
            new SocialError(
              error.code,
              'firestore/getUserProviderData ' + error.message,
            ),
          );
        });
    });
