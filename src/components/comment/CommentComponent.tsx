...
> {
  ...
  render() {
    ...
    const commentBody = (
      <div
        style={{outline: 'none', flex: 'auto', flexGrow: 1, marginTop: '-14px'}}
      >
        {editorStatus ? (
          <TextField
          ...
          <div className={classNames('animate2-top10')}>
            <h6 className={classes.comment}>{this.state.text}</h6>
          </div>
        )}
      ...
      <div className="animate-top" key={comment.id!}>
        <Card elevation={0} className={classes.root}>
          <CardHeader
            ...
            avatar={
              <NavLink to={`/${userId}`}>
                <UserAvatar fullName={fullName!} fileName={avatar!} size={35} />
              </NavLink>
            }
            action={
              (!this.props.isCommentOwner &&
          ...
          <CardActions className={classes.cardAction}>
            ...
            <IconButton aria-label="Share" onClick={this.handleDownVote}>
...
const mapDispatchToProps = (
  ...
  return {
    ...
    upvoteComment: (reverse: boolean) =>
      dispatch(
        voteActions.addVoteComment(
          ownProps.comment.postId,
          ownProps.comment.id,
          reverse,
        ),
      ),
    downVoteComment: (reverse: boolean) =>
      dispatch(
        voteActions.downVoteComment(
          ownProps.comment.postId,
          ownProps.comment.id,
          reverse,
        ),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(styles as any)(CommentComponent as any) as any);
