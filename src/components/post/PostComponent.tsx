...
> {
  ...
  handleOpenShare = () => {
    ...
    copy(
      `${location.origin}/posts/${post.get('ownerUserId')}/${post.get('id')}`,
    );
    this.setState({
      shareOpen: true,
    });
  ...
  shouldComponentUpdate(
    nextProps: IPostComponentProps,
    nextState: IPostComponentState,
  ) {
    ...
    const rightIconMenu = (
      <div>
        ...
        >
          <MenuItem
            onClick={() =>
              this.props.toggleDisableComments!(!post.get('disableComments'))
            }
          >
          ...
          {isAdmin === true ? (
            <div>
              <MenuItem onClick={() => AdminAPi.addToFeatured(post.get('id'))}>
                {'Add to featured'}
              </MenuItem>
              <MenuItem
      ...
      <div className={classes.cardBottemContent}>
        ...
        <div className={classes.textArea}>
          <NavLink to={`/posts/${post.get('ownerUserId')}/${post.get('id')}`}>
            <p className={classes.MainBody}>{post.get('bodyText')}</p>
          </NavLink>
        </div>
      </div>
    );
    ...
    return (
      <Card key={`post-component-${post.get('id')}`} className={classes.Card}>
        <div className={classes.header}>
          <CardHeader action={isPostOwner ? rightIconMenu : ''}></CardHeader>
        </div>
              ...
              <div style={{color: '#8b9898'}}>
                <span>
                  {post.get('creationDate') ? (
                    moment.unix(post.get('creationDate')).fromNow() +
                    ' | ' +
          ...
          <div style={{display: 'inherit'}}>
            ...
            >
              ...
              <div className={classes.commentCounter}>
                {' '}
                {post.get('commentCounter')! > 0
                  ? post.get('commentCounter')
                  : ''}{' '}
              </div>
            </IconButton>
          </div>
...
const mapDispatchToProps = (dispatch: any, ownProps: IPostComponentProps) => {
  ...
  return {
    ...
    delete: (id: string) =>
      dispatch(postActions.dbDeletePost(id, tag ? tag : 'userPosts')),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withStyles(PostPreview as any)(PostComponent as any) as any);
