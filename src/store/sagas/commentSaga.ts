...
function* setComments(
  ...
  const post: Map<string, any> = yield select(
    postSelector.getPost,
    ownerId,
    postId,
  );
  if (post) {
    ...
    if (desiredComments) {
      ...
      const updatedPost = post
        .set('comments', Map(sortedObjects))
...
function* dbFetchComments(ownerId: string, postId: string) {
  ...
  const channelSubscription: Channel<Map<
    string,
    Map<string, any>
  >> = yield call(fetchCommentsChannel, postId);
