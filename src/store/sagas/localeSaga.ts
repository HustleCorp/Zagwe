...
function* initLocalization() {
  const languages = [
    ...
    {name: 'Amharic', code: LanguageType.Amharic},
  ];
  yield put(initialize(languages));
  // To set a different default active language set the `defaultLanguage` option.
  yield put(
    initialize(languages, {defaultLanguage: config.settings.defaultLanguage}),
  );
  const englishLocale = require('locale/en.json');
  const amharicLocale = require('locale/am.json');
  yield put(addTranslationForLanguage(englishLocale, LanguageType.English));
  yield put(addTranslationForLanguage(amharicLocale, LanguageType.Amharic));
}
