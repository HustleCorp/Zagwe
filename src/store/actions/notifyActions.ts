 ...
 */
const notificationService: INotificationService = provider.get<
...
export const sendTokenTodb = (userId: string, token: string) => {
  return notificationService
        ...
        (notifications: {[notifyId: string]: Notification}) => {
          Object.keys(notifications).forEach(key => {
            if (
              ...
              dispatch(
                userActions.dbGetUserInfoByUserId(
                  notifications[key].notifierUserId,
                  '',
                ),
              );
            }
