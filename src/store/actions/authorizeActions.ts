 ...
 * @param {string} uids
 */
export const login = (uid: string, isVerifide: boolean, isAdmin: boolean) => {
  return {
    type: AuthorizeActionType.LOGIN,
    payload: {authed: true, isVerifide, uid, isAdmin},
  };
};
 ...
 */
const authorizeService: IAuthorizeService = provider.get<IAuthorizeService>(
...
export const dbLogout = () => {
  return (dispatch: any, getState: any) => {
    return authorizeService.logout().then(
      result => {
...
export const dbLoginWithOAuth = (type: OAuthType) => {
  return (dispatch: any, getState: any) => {
    ...
    return authorizeService
      .loginWithOAuth(type)
      .then(async (result: LoginUser) => {
        // Send email verification successful.
        dispatch(globalActions.showNotificationSuccess());
        dispatch(login(result.uid, true, result.isAdmin));
        dispatch(push('/'));
      })
      .catch((error: SocialError) => {
        // An error happened.
