 ...
 */
const circleService: ICircleService = provider.get<ICircleService>(
  SocialProviderTypes.CircleService,
);
const userTieService: IUserTieService = provider.get<IUserTieService>(
  SocialProviderTypes.UserTieService,
);
 ...
 */
export const dbFollowUser = (
  followingCircleId: string,
  userFollowing: UserTie,
) => {
 ...
 */
export let dbUpdateUserInCircles = (
  circleIdList: List<string>,
  userFollowing: UserTie,
) => {
  return (dispatch: any, getState: Function) => {
    const state: Map<string, any> = getState();
...
export let dbDeleteFollowingUser = (userFollowingId: string) => {
  return (dispatch: any, getState: Function) => {
    ...
    // Set server request status to {Sent}
    const deleteFollowingUserRequest = createdeleteFollowingUserRequest(
      userFollowingId,
    );
...
export const dbUpdateCircle = (newCircle: Circle) => {
  return (dispatch: any, getState: Function) => {
    ...
    let circle: Map<string, any> = state.getIn([
      'circle',
      'circleList',
      newCircle.id!,
    ]);
    ...
    return circleService
      .updateCircle(uid, newCircle.id!, circle.toJS() as any)
      .then(
        () => {
...
export const dbGetFollowers = () => {
  return (dispatch: any, getState: Function) => {
    ...
    if (uid) {
      ...
      dispatch(userActions.dbGetUserInfo()).catch((error: SocialError) => {
        dispatch(globalActions.showMessage(error.message));
      });
    }
  };
