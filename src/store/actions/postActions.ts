...
export const dbGetPostbyTopic = (
  ...
  return (dispatch: any, getState: Function) => {
    ...
    if (lastPageRequest !== page) {
      return postService
        .getAllPostsByTopic(header, lastPostId, page, limit)
        .then(result => {
...
export const dbGetTopPosts = (page: number = 0, limit: number = 5) => {
  return (dispatch: any, getState: Function) => {
    const state: Map<string, any> = getState();
    const stream: Map<string, any> = state.getIn([
      'post',
      'stream',
      'userPosts',
    ]);
    ...
    if (lastPageRequest !== page) {
      return postService
        .getAllPosts(lastPostId, page, limit)
        .then(result => {
          if (!result.posts || !(result.posts.length > 0)) {
...
export const dbGetFollowingPosts = (page: number = 0, limit: number = 5) => {
  return (dispatch: any, getState: Function) => {
    ...
    if (uid && lastPageRequest !== page) {
      return postService
        .getPosts(uid, lastPostId, page, limit)
        .then(result => {
          if (!result.posts || !(result.posts.length > 0)) {
 ...
 * Get all user posts from data base
 */
export const dbGetPostsByUserId = (
  userId: string,
  page: number = 0,
  limit: number = 5,
) => {
  return (dispatch: any, getState: Function) => {
    ...
    const lastPageRequest = state.getIn(
      ['post', 'profile', userId, 'lastPageRequest'],
      -1,
    );
    const lastPostId = state.getIn(
      ['post', 'profile', userId, 'lastPostId'],
      '',
    );
    let uid: string = state.getIn(['authorize', 'uid']);
 ...
 * Add Following post
 */
export const addTopicPosts = (
  header: string,
  followingPosts: Map<string, Map<string, any>>,
) => {
  return {
