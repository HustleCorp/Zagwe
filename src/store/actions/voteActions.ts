 ...
 */
const voteService: IVoteService = provider.get<IVoteService>(
  SocialProviderTypes.VoteService,
);
const commentService: ICommentService = provider.get<ICommentService>(
  SocialProviderTypes.CommentService,
);

/* _____________ CRUD DB _____________ */

export const addVoteComment = (
  ownerPostId: any,
  commentId: any,
  reverse: boolean,
) => {
  return (dispatch: any, getState: Function) => {
    ...
    const comment: Map<string, any> = state.getIn([
      'comment',
      'postComments',
      ownerPostId,
      commentId,
    ...
    const votedComment = reverse
      ? (comment
          .set('upvote', score)
          .deleteIn(['upvoted', uid])
          .toJS() as Comment)
      : (comment
          .set('upvote', score)
          .setIn(['upvoted', uid], true)
          .toJS() as Comment);
...
export const downVoteComment = (
  ...
  return (dispatch: any, getState: Function) => {
    ...
    const votedComment = reverse
      ? (comment
          .set('downvote', score)
          .deleteIn(['downvoted', uid])
          .toJS() as Comment)
      : (comment
          .set('downvote', score)
          .setIn(['downvoted', uid], true)
          .toJS() as Comment);
...
export const dbAddVote = (postId: string, ownerPostUserId: string) => {
  return (dispatch: any, getState: Function) => {
    ...
    const post: Map<string, any> = state.getIn([
      'post',
      'userPosts',
      ownerPostUserId,
      postId,
    ...
    return voteService
      ...
      .then((voteKey: string) => {
        if (uid !== ownerPostUserId) {
          dispatch(
            notifyActions.dbAddNotification({
 ...
 */
export const addVoteList = (votes: {
  [postId: string]: {[voteId: string]: Vote};
}) => {
