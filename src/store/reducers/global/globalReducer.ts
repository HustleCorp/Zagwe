...
export const globalReducer = (
  ...
  switch (action.type) {
    ...
    case GlobalActionType.SHOW_NORMAL_MESSAGE_GLOBAL:
      return state.set('message', action.payload).set('messageOpen', true);
    ...
    case GlobalActionType.HIDE_TOP_LOADING:
      const queueTopLoading =
        Number(state.get('topLoadingQueue')) > 0
          ? Number(state.get('topLoadingQueue')) - 1
          : 0;
      return state
    ...
    case GlobalActionType.HIDE_MASTER_LOADING:
      const queueMasterLoading =
        Number(state.get('masterLoadingQueue')) > 0
          ? Number(state.get('masterLoadingQueue')) - 1
          : 0;
      return state
