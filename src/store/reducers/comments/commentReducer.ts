 ...
 * @param action
 */
export let commentReducer = (
  state = Map(new CommentState()),
  ...
  switch (action.type) {
    ...
    case CommentActionType.ADD_COMMENT:
      ...
      return state.setIn(
        ['postComments', comment.get('postId'), comment.get('id')],
        comment,
      );

    case CommentActionType.ADD_COMMENT_LIST:
      return state.mergeIn(['postComments'], payload).set('loaded', true);

    case CommentActionType.UPDATE_COMMENT:
      const upload = state.setIn(
        ['postComments', payload.postId, payload.id],
        Map(payload),
      );
      return upload;
