...
const updatePost = (state: any, payload: any) => {
  ...
  return state.setIn(['userPosts', updatePostOwnerId, updatePostId], Map(post));
};
...
const updatePostComments = (state: any, payload: any) => {
  ...
  const updatePostId = post.get('id');
  return state.setIn(
    ['userPosts', updatePostOwnerId, updatePostId, 'comments'],
    post.get('comments'),
  );
};
...
const updatePostVotes = (state: any, payload: any) => {
  ...
  const updatePostId = post.get('id');
  return state.setIn(
    ['userPosts', updatePostOwnerId, updatePostId, 'votes'],
    post.get('votes'),
  );
};
...
export let postReducer = (
  ...
  switch (action.type) {
    ...
    case PostActionType.ADD_IMAGE_POST:
      return state.setIn(
        ['userPosts', payload.uid, payload.post.id],
        Map(payload.post),
      );
    ...
    case PostActionType.DELETE_POST:
      return state.deleteIn([payload.tag, payload.uid, payload.id]);
    ...
    case PostActionType.NOT_MORE_DATA_STREAM:
      return state.setIn(['stream', payload.type, 'hasMoreData'], false);
    case PostActionType.REST_LAST_POST_ID:
      return state.setIn(['stream', payload.type, 'lastPostId'], '');
    ...
    case PostActionType.LAST_POST_STREAM:
      return state.setIn(
        ['stream', payload.type, 'lastPostId'],
        payload.lastPostId,
      );
    ...
    case PostActionType.LAST_POST_PROFILE:
      return state.setIn(
        ['profile', payload.userId, 'lastPostId'],
        payload.lastPostId,
      );
