 ...
 */
export let imageGalleryReducer = (
  state = Map(new ImageGalleryState()),
  action: IImageGalleryAction,
) => {
  ...
  switch (action.type) {
    ...
    case ImageGalleryActionType.ADD_IMAGE_GALLERY:
      return state.mergeIn(['images'], List([payload]));

    case ImageGalleryActionType.ADD_IMAGE_LIST_GALLERY:
      return state.set('images', List(payload)).set('loaded', true);

    case ImageGalleryActionType.DELETE_IMAGE:
      return state.update('images', (images: List<Image>) => {
