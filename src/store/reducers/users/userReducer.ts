...
export let userReducer = (
  state = Map(new UserState()),
  action: IUserAction,
) => {
  const {payload} = action;
  switch (action.type) {
    case UserActionType.USER_INFO:
      return state.setIn(['info', payload.uid], payload.info);
    case UserActionType.USER_OTHER:
      return;
    ...
    case UserActionType.ADD_USER_OTHER:
      return state
        .setIn(['otherInfo', payload.uid], payload.info)
        .set('otherloaded', true);

    case UserActionType.ADD_PEOPLE_INFO:
      return state.mergeIn(['info'], payload);

    case UserActionType.UPDATE_USER_INFO:
      return state.mergeIn(['info', payload.uid], payload.info);

    case UserActionType.UPDATE_USER_OTHER:
      return state.mergeIn(['otherInfo', payload.uid], payload.info);
    ...
    case UserActionType.HAS_MORE_DATA_PEOPLE:
      return state.setIn(['people', 'hasMoreData'], true);

    case UserActionType.NOT_MORE_DATA_PEOPLE:
      return state.setIn(['people', 'hasMoreData'], false);

    case UserActionType.REQUEST_PAGE_PEOPLE:
      return state.setIn(['people', 'lastPageRequest'], payload.page);

    case UserActionType.LAST_USER_PEOPLE:
      return state.setIn(['people', 'lastUserId'], payload.lastUserId);
