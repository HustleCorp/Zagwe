 ...
 * @param action
 */
export let circleReducer = (
  state = Map(new CircleState()),
  action: ICircleAction,
) => {
  ...
  switch (action.type) {
    ...
    case CircleActionType.ADD_FOLLOWING_USER:
      ...
      return state
        ...
        .setIn(
          ['selectedCircles', userTie.get('userId')],
          userTie.get('circleIdList'),
        );

    case CircleActionType.UPDATE_USER_TIE:
      return state.setIn(
        ['userTies', payload.userTie.user.userId],
        payload.userTie,
      );
    ...
    case CircleActionType.SET_SELECTED_CIRCLES_USER_BOX_COMPONENT:
      return state.setIn(
        ['selectedCircles', payload.userId],
        payload.circleList,
      );
    ...
    case CircleActionType.OPEN_SELECT_CIRCLES_USER_BOX_COMPONENT:
      return state.setIn(['openSelecteCircles', payload.userId], true);

    case CircleActionType.CLOSE_SELECT_CIRCLES_USER_BOX_COMPONENT:
      return state.setIn(['openSelecteCircles', payload.userId], false);
...
const getSelectedCircles = (userTies: Map<string, any>) => {
  ...
  userTies.forEach(userTie => {
    selectedCircles = selectedCircles.set(
      userTie.get('userId'),
      List(userTie.get('circleIdList')),
    );
  });
